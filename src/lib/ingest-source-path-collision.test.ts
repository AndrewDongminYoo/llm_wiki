...
describe("autoIngest source summary paths", () => {
  ...
  it("rejects unsafe traversal and system-owned aggregate paths from interactive writes", async () => {
    if (!tmp) throw new Error("missing temp project")
    // index.md / log.md are system-generated; case- and separator-variant
    // spellings must not evade the drop and collide with the real files on a
    // case-insensitive filesystem. ../escape.md is a path-traversal attempt.
    interactiveGenerationOverride = [
      "---FILE: wiki/INDEX.md---\n# hostile index\n---END FILE---",
      "---FILE: wiki/LOG.md---\n# hostile log\n---END FILE---",
      "---FILE: ../escape.md---\n# escape\n---END FILE---",
