...
- `POST /api/v1/projects/{id}/reviews/{reviewId}/resolve` — mark a Review item resolved (optional JSON body `{ "action": "label" }`); the Review tab's Refresh button re-reads the result from disk
